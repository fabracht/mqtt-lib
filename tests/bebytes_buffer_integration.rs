...
mod performance_tests {
    ...
    fn test_bebytes_vs_manual_buffer_performance() {
        ...
        let headers: Vec<_> = (0..4).map(|i| {
            MqttTypeAndFlags::for_publish((i % 3) as u8, i % 2 == 0, false)
        }).collect();
        ...
        assert!(ratio < 30.0, "BeBytes integration unexpectedly slow: {:.2}x", ratio);
    }
    ...
    fn test_bebytes_encode_be_to_performance() {
        ...
        println!("BeBytes encode_be_to:   {:?} ({:.2}x vs manual)", 
                 bebytes_encode_time, 
                 bebytes_encode_time.as_nanos() as f64 / manual_time.as_nanos() as f64);
        println!("BeBytes to_bytes_buf:   {:?} ({:.2}x vs manual)", 
                 bebytes_buf_time,
        ...
        assert!(buf_ratio < encode_ratio, "to_be_bytes_buf should be faster than encode_be_to");
    }
}