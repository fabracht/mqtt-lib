...
use mqtt_v5::packet::subscribe::{SubscriptionOptionsBits, SubscriptionOptions, RetainHandling};
...
fn test_mqtt_type_and_flags_raw_pointer_eligibility() {
    ...
    println!("MqttTypeAndFlags to_be_bytes_buf: {} bytes", buf_bytes.len());
...
fn test_subscription_options_raw_pointer_eligibility() {
    ...
    println!("SubscriptionOptionsBits to_be_bytes_buf: {} bytes", buf_bytes.len());
    
    // Test standard to_be_bytes for comparison
    let std_bytes = options.to_be_bytes();
    println!("SubscriptionOptionsBits to_be_bytes: {} bytes", std_bytes.len());
...
fn test_ping_packets_raw_pointer_eligibility() {
    ...
    println!("PingResponse to_be_bytes_buf: {} bytes", resp_buf_bytes.len());
...
fn test_performance_characteristics() {
    ...
    println!("  - MqttTypeAndFlags: {} bytes", type_flags.to_be_bytes_buf().len());
    println!("  - SubscriptionOptionsBits: {} bytes", sub_opts.to_be_bytes_buf().len());
    ...
    println!("  - AckPacketHeader: {} bytes", ack_header.to_be_bytes_buf().len());
    println!("  - PingRequest: {} bytes", ping_req.to_be_bytes_buf().len());
...
mod raw_pointer_benchmarks {
    ...
    fn bench_single_byte_structures() {
        ...
        println!("  Speedup: {:.2}x", std_time.as_nanos() as f64 / buf_time.as_nanos() as f64);
    }
