...
async fn test_message_collector_advanced() {
    use common::{MessageCollector, wait_with_message};
    ...
    for i in 0..5 {
        client
            .publish(&format!("test/collector/{}", i), format!("Message {}", i).as_bytes())
            .await
    ...
    for (i, msg) in messages.iter().enumerate() {
        ...
        assert_eq!(String::from_utf8_lossy(&msg.payload), format!("Message {}", i));
    }
...
async fn test_event_counter_with_wildcards() {
    ...
    client.publish("events/sensor1/data", b"data1").await.unwrap();
    client.publish("events/sensor2/data", b"data2").await.unwrap();
    client.publish("events/sensor3/data", b"data3").await.unwrap();
    client.publish("events/other/info", b"should not count").await.unwrap(); // Won't match
