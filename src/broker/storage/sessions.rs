...
impl<B: StorageBackend> SessionManager<B> {
    ...
    pub async fn update_subscriptions(
        ...
        if let Some(mut session) = self.storage.get_session(client_id).await {
            ...
            debug!("Attempted to update subscriptions for non-existent session: {}", client_id);
            Ok(())
        }
    }
    
    /// Add subscription to session
    pub async fn add_subscription(&self, client_id: &str, topic_filter: &str, qos: QoS) -> Result<()> {
        if let Some(mut session) = self.storage.get_session(client_id).await {
            session.add_subscription(topic_filter.to_string(), qos);
            session.touch();
            
            debug!("Added subscription {} for client: {}", topic_filter, client_id);
            self.storage.store_session(session).await
        } else {
            Ok(())
        }
    ...
    pub async fn remove_subscription(&self, client_id: &str, topic_filter: &str) -> Result<()> {
        if let Some(mut session) = self.storage.get_session(client_id).await {
            ...
            debug!("Removed subscription {} for client: {}", topic_filter, client_id);
            self.storage.store_session(session).await
        } else {
            Ok(())
        }
