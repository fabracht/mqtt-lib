...
impl AclManager {
    ...
    pub async fn load_acl_file(&self) -> Result<()> {
        ...
        let content = fs::read_to_string(path).await.map_err(|e| {
            MqttError::Configuration(format!("Failed to read ACL file {}: {}", path.display(), e))
        })?;
        ...
        for line in content.lines() {
            ...
            let permission = match parts[5].parse::<Permission>() {
                Ok(p) => p,
                Err(e) => {
                    warn!("Invalid permission at line {}: {} - {}", line_num, parts[5], e);
                    continue;
                }
            };
        ...
        *self.rules.write().await = rules;
        ...
        info!("Loaded {} ACL rules from file: {}", self.rules.read().await.len(), path.display());
        Ok(())
    }
    ...
    pub async fn check_publish(&self, username: Option<&str>, topic: &str) -> bool {
        self.check_permission(username, topic, |p| p.allows_write()).await
    }
    ...
    pub async fn check_subscribe(&self, username: Option<&str>, topic_filter: &str) -> bool {
        self.check_permission(username, topic_filter, |p| p.allows_read()).await
    }
        ...
        F: Fn(Permission) -> bool,
    {
        ...
        for rule in rules.iter() {
            if rule.matches(username, topic) {
                ...
                let wildcard_count = rule.topic_pattern.chars().filter(|&c| c == '+' || c == '#').count();
                specificity += 50 - (wildcard_count as i32 * 10);
                
                // Topic length as tiebreaker (longer = more specific)
                specificity += rule.topic_pattern.len() as i32;
        ...
        if let Some(rule) = best_match {
            debug!("ACL rule matched: user={:?}, topic={}, permission={:?}, specificity={}", 
        ...
        debug!("No ACL rule matched for user={:?}, topic={}, using default permission={:?}", 
...
mod tests {
    ...
    fn test_permission_parsing() {
        ...
        assert_eq!("readwrite".parse::<Permission>().unwrap(), Permission::ReadWrite);
    ...
    fn test_acl_rule_matching() {
        let rule = AclRule::new("alice".to_string(), "sensors/+".to_string(), Permission::Read);
        ...
        // Wildcard user rule
        let wildcard_rule = AclRule::new("*".to_string(), "public/#".to_string(), Permission::ReadWrite);
    ...
    async fn test_acl_basic_operations() {
        ...
        acl.add_rule(AclRule::new("alice".to_string(), "sensors/+".to_string(), Permission::Read)).await;
        
        assert!(!acl.check_publish(Some("alice"), "sensors/temp").await);
        assert!(acl.check_subscribe(Some("alice"), "sensors/temp").await);
        
        // Add rule allowing Bob to write to actuators
        acl.add_rule(AclRule::new("bob".to_string(), "actuators/#".to_string(), Permission::Write)).await;
        
        assert!(acl.check_publish(Some("bob"), "actuators/fan/speed").await);
        assert!(!acl.check_subscribe(Some("bob"), "actuators/fan/speed").await);
    ...
    async fn test_acl_rule_priority() {
        ...
        acl.add_rule(AclRule::new("*".to_string(), "data/#".to_string(), Permission::ReadWrite)).await;
        
        // Add specific deny rule
        acl.add_rule(AclRule::new("alice".to_string(), "data/secret/#".to_string(), Permission::Deny)).await;
        ...
        assert!(!acl.check_subscribe(Some("alice"), "data/secret/file1").await);
        
        // Alice should have access to other data
        assert!(acl.check_publish(Some("alice"), "data/public/file1").await);
        assert!(acl.check_subscribe(Some("alice"), "data/public/file1").await);
    ...
    async fn test_acl_allow_all_manager() {
        ...
        // Add specific deny rule
        acl.add_rule(AclRule::new("alice".to_string(), "forbidden/#".to_string(), Permission::Deny)).await;
    ...
    async fn test_acl_file_loading() {
        ...
        assert!(!acl.check_publish(Some("alice"), "sensors/temp").await);
        assert!(acl.check_subscribe(Some("alice"), "sensors/temp").await);
        assert!(acl.check_publish(Some("alice"), "public/announcements").await);
