...
impl SysTopicsProvider {
    ...
    async fn publish_static_topics(&self) {
        // Broker version
        self.publish_sys_topic("$SYS/broker/version", &self.version).await;
        
        // Broker implementation
        self.publish_sys_topic("$SYS/broker/implementation", "mqtt-v5").await;
        
        // Protocol version
        self.publish_sys_topic("$SYS/broker/protocol_version", "5.0").await;
    }
    ...
    async fn publish_dynamic_topics(&self) {
        ...
        self.publish_sys_topic("$SYS/broker/uptime", &uptime.to_string()).await;
        
        // Client statistics
        let connected = self.stats.clients_connected.load(Ordering::Relaxed);
        self.publish_sys_topic("$SYS/broker/clients/connected", &connected.to_string()).await;
        
        let total = self.stats.clients_total.load(Ordering::Relaxed);
        self.publish_sys_topic("$SYS/broker/clients/total", &total.to_string()).await;
        
        let maximum = self.stats.clients_maximum.load(Ordering::Relaxed);
        self.publish_sys_topic("$SYS/broker/clients/maximum", &maximum.to_string()).await;
        
        // Message statistics
        let msg_sent = self.stats.messages_sent.load(Ordering::Relaxed);
        self.publish_sys_topic("$SYS/broker/messages/sent", &msg_sent.to_string()).await;
        
        let msg_recv = self.stats.messages_received.load(Ordering::Relaxed);
        self.publish_sys_topic("$SYS/broker/messages/received", &msg_recv.to_string()).await;
        
        let pub_sent = self.stats.publish_sent.load(Ordering::Relaxed);
        self.publish_sys_topic("$SYS/broker/messages/publish/sent", &pub_sent.to_string()).await;
        
        let pub_recv = self.stats.publish_received.load(Ordering::Relaxed);
        self.publish_sys_topic("$SYS/broker/messages/publish/received", &pub_recv.to_string()).await;
        
        // Byte statistics
        let bytes_sent = self.stats.bytes_sent.load(Ordering::Relaxed);
        self.publish_sys_topic("$SYS/broker/bytes/sent", &bytes_sent.to_string()).await;
        
        let bytes_recv = self.stats.bytes_received.load(Ordering::Relaxed);
        self.publish_sys_topic("$SYS/broker/bytes/received", &bytes_recv.to_string()).await;
        
        // Router statistics
        let retained_count = self.router.retained_count().await;
        self.publish_sys_topic("$SYS/broker/retained/count", &retained_count.to_string()).await;
        
        let topic_count = self.router.topic_count().await;
        self.publish_sys_topic("$SYS/broker/subscriptions/count", &topic_count.to_string()).await;
    }
    
    /// Publish a single $SYS topic
    async fn publish_sys_topic(&self, topic: &str, value: &str) {
        let packet = PublishPacket::new(topic, value.as_bytes(), QoS::AtMostOnce)
            .with_retain(true);
